...
class Realm {
  RealmConfiguration
      realmConfiguration; // to be injected with the one generated at compile time
  ...
  T create<T extends RealmModel>([T obj]) {
    var proxyInstance = realmConfiguration.newProxyInstance<T>(T);

    final objectTypeC = Utf8.toUtf8(proxyInstance.tableName);
    var objectPointer =
        bindings.wrapper_add_object(_databasePointer, objectTypeC);
    free(objectTypeC);
  ...
  Future<void> delete<T extends RealmModel>(T objectInstance) async {
    bindings.wrapper_delete_object(
        _databasePointer, objectInstance.objectPointer);
  }

  Future<RealmResults<T>> objects<T extends RealmModel>(String query) async {
    //TODO make query optional, so passing nothing ie equivalent to findAll
    // create RealmResults which holds the query results
    var realmresultsInstance = realmConfiguration.newRealmResultsInstance<T>(T);

    final objectTypeC = Utf8.toUtf8(realmresultsInstance.tableName);
    final queryC = Utf8.toUtf8(query);
    realmresultsInstance.nativePointer =
        bindings.wrapper_query(_databasePointer, objectTypeC, queryC);
