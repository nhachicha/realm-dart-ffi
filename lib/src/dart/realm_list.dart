...
class RealmList<T extends RealmModel> extends ListBase<T> {
  ...
  void add(T value) {
    ...
    if (value.isManaged) {
      ...
      var persisted = realm.create<T>(value);
      // persist values
      nativePointer = persisted.objectPointer;
    }
    bindings.wrapper_realmlist_insert(
        nativeRealmListPointer, nativePointer, length);
  }
  ...
  void insert(int index, T item) {
    ...
    bindings.wrapper_realmlist_insert(
        nativeRealmListPointer, item.objectPointer, index);
  }
  ...
  T operator [](int index) {
    ...
    var realmObjectPointer = bindings.wrapper_realmlist_get(
        nativeRealmListPointer, tableNameC, index);
    free(tableNameC);
  ...
  void operator []=(int index, T value) {
    ...
    final tableNameC = Utf8.toUtf8(tableName);
    bindings.wrapper_realmlist_set(
        nativeRealmListPointer, nativePointer, index);
    free(tableNameC);
  ...
  set length(int newLength) {
    throw Exception(
        'Modifying length is not supported'); // Realm list items cannot be null
  }
}
